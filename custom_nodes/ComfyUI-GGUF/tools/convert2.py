...
from gguf_connector import writer as gw
...
def handle_tensors(writer, state_dict, model_arch):
    ...
    for key, data in tqdm(state_dict.items()):
        ...
        if old_dtype == torch.bfloat16:
            data_qtype = gw.GGMLQuantizationType.BF16
        # elif old_dtype == torch.float32:
        #     data_qtype = gw.GGMLQuantizationType.F32
        else:
        ...
        except (AttributeError, ggc.QuantError) as e:
...
def convert_file(path, dst_path=None, interact=True, overwrite=False):
    ...
    if main_dtype == torch.bfloat16:
        ftype_name = "BF16"
        ftype_gguf = gw.LlamaFileType.MOSTLY_BF16
    # elif main_dtype == torch.float32:
    #     ftype_name = "F32"
    #     ftype_gguf = None
    else:
        ftype_name = "F16"
        ftype_gguf = gw.LlamaFileType.MOSTLY_F16
    ...
    # handle actual file
    writer = gw.GGUFWriter(path=None, arch=model_arch.arch)
